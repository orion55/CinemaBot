...
namespace CinemaBot.Services.Services
{
    public class ProxyService
    {
        ...
        private void Init()
        {
            ...
            int count = File.ReadLines(proxyFullname).Count();
        ...
        {
            var proxyList = _proxies.Where(proxy => proxy.IsBad);
            IEnumerable<Proxy> list = proxyList as Proxy[] ?? proxyList.ToArray();
            if (list.Any())
            {
                  SaveFileProxy(list, BadProxyFilename);
            }
        }

        private void SaveFileProxy(IEnumerable<Proxy> list, string filename)
        {
